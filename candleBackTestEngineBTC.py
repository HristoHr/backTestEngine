...
potentialLoss = 0
potentialProfit = 0
profitWantedDefault = 0.0025
profitWanted = 0.0025
ATRFilterLow = 0.0000
ATRFilterHigh = 1.00055
ATRList = []
DCLowList = []
candleOpenList = []
candleCloseList = []
...
cur = conn.cursor()
cur.execute("SELECT * FROM candles_BTC_LTC WHERE start >?", (1420160461, ))
    ...
    if (candleCount >= historySize):
        ...
        if (action == "sell" or action == "cancelBuy"):
            if (candleOpen < SMA and ATR < ATRFilterHigh and ATR > ATRFilterLow):
                ...
                if (profitWanted * 10 < lossFee * equityAlt):
                    feeHigh += 1
                else:
                    action = "openBuy"
                    # print("Open Buy at:", candleOpen)
                    # print("Wanted Profit:", profitWanted)
                    # print("Alts bought:", equityAlt)
